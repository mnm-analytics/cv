...
`Docker` `Apache` `Tomcat` `SAS` `Mecab` `Cabocha` `TreeTagger` `Hadoop`
...
`Python` `MeCab` `AWS Traslate` `Google Translate`
...
【発揮したバリュー】
- 単純に和文・英文の品詞頻度を比較しても相違がでて当然なので、AWSやGCPの翻訳APIで相互に翻訳し、それぞれの言語の原文と品詞頻度を比較するという提案を行い合意を得たことで、プロモーションの前進に貢献した。
